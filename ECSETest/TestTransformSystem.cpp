...
{
    transA->setNextPosition(sf::Vector2f(200.f, 100.f));
    transA->advance();

    ASSERT_EQ(transA->getPosition(), system->getPosition(*a));
}
...
TEST_F(TransformSystemTest, ChildConvertLocalPositionTest)
{
    ...
    transA->setNextPosition(globalPos);
    transA->advance();

    transB->setNextPosition(sf::Vector2f(8.f, 2.f));
    transB->advance();

    system->parentEntity(*a, *b);

    auto localPos = transA->getPosition();
    auto newGlobalPos = system->getPosition(*a);
...
TEST_F(TransformSystemTest, ChildConvertLocalAngleTest)
{
    ...
    transA->setNextAngle(globalAngle);
    transA->advance();

    transB->setNextAngle(10.f);
    transB->advance();

    system->parentEntity(*a, *b);

    ASSERT_NEAR_TRANS(globalAngle, system->getAngle(*a));
    ASSERT_NEAR_TRANS(15.f, transA->getAngle());
}
...
TEST_F(TransformSystemTest, ChildConvertLocalAnglePositionTest)
{
    ...
    transB->setNextPosition(sf::Vector2f(1.f, 2.f));
    transB->setNextAngle(ECSE::halfPi);
    transB->advance();
    ...
    ASSERT_NEAR_TRANS(-ECSE::quarterPi, transA->getAngle());
}
...
TEST_F(TransformSystemTest, ChildRevertLocalPositionTest)
{
    ...
    transB->setNextPosition(sf::Vector2f(-12.f, 7.f));
    transB->advance();
...
TEST_F(TransformSystemTest, ChildRevertLocalAngleTest)
{
    ...
    ASSERT_NEAR_TRANS(globalAngle, system->getAngle(*a));
    ASSERT_NEAR_TRANS(globalAngle, transA->getAngle());
}
...
TEST_F(TransformSystemTest, ChildRevertLocalAnglePositionTest)
{
    ...
    transB->setNextPosition(sf::Vector2f(4.f, 5.f));
    transB->setNextAngle(-23.f);
    transB->advance();
